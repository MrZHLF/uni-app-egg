		...
		<view class="flex flex-wrap">
			<view class="list-item" v-for="(item,index) in list" :key="index" @click="openLive(item.id)">
				...
				<view class="rounded-circle px-2 flex align-center" style="position: absolute;left: 15rpx;top: 15rpx;background-color: rgba(0,0,0,0.4);">
					...
					<text class="text-white font-sm">{{item.coin}}</text>
				</view>
				...
				<view class="rounded-circle  flex align-center" style="position: absolute;bottom: 15rpx;left: 15rpx;">
					<text class="font-sm text-white">{{item.title}}</text>
				</view>
				
				<view class="rounded-circle px-2 flex align-center" style="position: absolute;right: 15rpx;bottom: 15rpx;right: 15rpx; background-color: rgba(0,0,0,0.4);">
					<view class="rounded-circle  mr-1" style="width: 20rpx;height: 20rpx;" :class="item.status === 1 ? 'bg-success' : 'bg-danger' "></view>
					<text class="font-sm text-white">{{item.status | formtStatus}}</text>
				</view>
